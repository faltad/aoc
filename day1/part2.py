...
def calc_distance(d, starting_position="north"):
    ...
    #if we see the map as a simple plan, with the point(0,0) being our starting point
    #we can plot all already visited points with simple X,Y values, X being the west->east
    #and Y south->north.
    visited = set([(0, 0)])
    ...
    for step in data:
        ...
        #now we need to walk one point by one point
        while number:
            dict_directions[current_position] += 1            
            number -= 1
            current_point = (dict_directions["east"] - dict_directions["west"], dict_directions["north"] - dict_directions["south"])
            if current_point in visited:
                results(dict_directions, d)
                return
            else:
